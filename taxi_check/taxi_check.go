...
// sequential run - processed 10 files in 7.045501131s
func main() {
	start := time.Now()
	rootDir := "/Users/randheer.chauhan/codebase/go-workshop/taxi_check/taxi"
	...
	for name, signature := range sigs {
		wg.Add(1)
		go func(name, signature string, ch chan bool) {
			defer wg.Done()
